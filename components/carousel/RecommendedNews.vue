    ...
    >
      <a :href="recommended.path" class="recommended-card">
        <div
          ...
          :data-type="computedNewsType(recommended.type)"
        >
          <img
...
import { computedNewsType, handleGetRandomElements } from '@/utils/functions';
import type { IRecommemded } from '@/utils/interface';

// fetch content navigation
const { data: navigation } = await useAsyncData(
  'navigation',
  () => fetchContentNavigation(queryContent('news', 'posts')),
  {
    lazy: true,
    transform: (rawData) => {
